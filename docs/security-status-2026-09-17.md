...
### 8.1 第二轮（指挥官：9 条全修且不影响功能）

指挥官点名 SkillSpector 那 **9 条**（含 4 条 AST4）要求全修且不影响功能 → 追加两项结构性改动 + 三处功能性修复：

| 项 | 处理 |
|---|---|
| **AST4 ×4 降为 1** | 外部进程调用全部收拢到 `svg_safety.run_external()`（唯一入口）：显式 `shell=False`、参数列表、硬超时、错误摘要。全仓 `subprocess.run` 调用点 **4 → 1**（`grep` 已验），命中数结构下降，且只剩一条审计路径 |
| **对比度误报（真功能 bug）** | 渐变填充（`fill="url(#g)"`）被当成"找不到背景矩形"→ **良性渐变 hero 直接判失败**。现区分 `[notice]`（不可机器判定，提示人工看渲染）与 `[issue]`（真失败）→ 渐变 hero 退出码 0 |
| **贴边误报（真功能 bug）** | 原"主导色"判据对渐变边缘误报（93% non-background）。改为**以频段中位色为基准 + 偏差 > 48/255 且占比 > 6%** 才判贴边 → 实测：渐变 hero 干净通过；纯色底 + 2px 红条跨顶边的样本**仍报 edge failure**（检测能力未削弱） |
| **相对路径 `--out` 会炸** | Chrome `--screenshot` 用相对路径时报"系统找不到指定的路径"；`--out` 现统一 `resolve()` 为绝对路径 |

**验证**：渐变 hero → 仅 `[notice]`、EXIT=0 ｜ 贴边样本 → `[edge]` failure、EXIT=1 ｜ 相对路径 `--out` → 正常出图 ｜ 冒烟全量 PASS（闸门 8 类全拒 / 良性放行 / 金丝雀 0 命中 / 渲染有内容）
