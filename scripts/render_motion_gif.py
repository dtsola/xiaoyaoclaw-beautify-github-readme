...
Trust boundary: the input SVG (and every layer derived from it) is untrusted
repository content. It is validated by scripts/svg_safety.py before any native
renderer or ffmpeg sees it, and all external processes run with shell=False
...
# run_external() is the only process-launching path used by this script.
try:
    from svg_safety import assert_safe_to_render, run_external
...
# Every external process (rsvg-convert / sips / ffmpeg) runs under a hard
# timeout so malformed or computationally expensive input cannot hang the run.
RENDER_TIMEOUT = 120
ENCODE_TIMEOUT = 300
...
def render_svg(
    ...
    """Render one SVG layer with the native renderer.

    The layer file is generated by this script from an already-gated document,
    but it is re-checked here too (defence in depth): a native renderer must
    never receive content carrying scripts, foreignObject, remote fonts or
    external/absolute references. The command is an explicit argument list
    built from validated paths and is launched through the shared
    `run_external()` funnel (shell=False, hard timeout).
    """
    assert_safe_to_render(svg_path)
    ...
    run_external(command, timeout=RENDER_TIMEOUT, label=f"{name} render")
...
def encode_gif(
    ...
    """Encode PNG frames into a GIF with ffmpeg.

    Every value that reaches the ffmpeg argument list is derived locally:
    `frames_dir` is a directory this script created, the input pattern is
    built from that directory, and all numeric parameters were range-checked
    by validate_spec() (fps 1-60, colors 2-256, dither from a fixed allow-list).
    Paths are therefore validated here before use, not interpolated from
    untrusted input, and the process runs with shell=False under a timeout.
    """
    frames_dir = frames_dir.resolve()
    if not (frames_dir.is_dir() and any(frames_dir.glob("frame-*.png"))):
        fail(f"frame directory is missing or empty: {frames_dir}")
    input_pattern = frames_dir / "frame-%04d.png"
    if input_pattern.parent != frames_dir:
        fail(f"frame pattern escapes the frame directory: {input_pattern}")
...
def run(args: argparse.Namespace) -> None:
    ...
    validate_spec(spec)
    # Trust boundary: the SVG may come from a third-party repository. Refuse
    # scripts, foreignObject, remote fonts or external/absolute references
    # before any native renderer or ffmpeg touches the content.
    assert_safe_to_render(input_svg)
