...
try:
    from svg_safety import assert_safe_to_render
...
def render_svg(
    ...
    external/absolute references. Argument lists are passed to subprocess with
    shell=False, so no shell interpretation happens, and every call is bounded
    by a hard timeout.
    """
    ...
    try:
        ...
        fail(f"renderer timed out after {RENDER_TIMEOUT}s on {svg_path.name}")
...
def encode_gif(
    ...
    try:
        ...
        fail(f"ffmpeg timed out after {ENCODE_TIMEOUT}s")
    if has_transparency:
